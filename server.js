...
// Configurações editáveis em runtime pelo admin (fonte da verdade para todos os handlers)
const CONFIG = {
...
async function getBscIncomingTxs(startBlock = 0, attempt = 1) {
  ...
  return new Promise((resolve) => {
    const path = `/api?module=account&action=txlist&address=${CONFIG.bscReceiverAddr}` +
...
async function checkBscDeposit(userId, chatId) {
  try {
    ...
    const processedHashes = new Set(
      (processedTxRows || [])
        .map(tx => { const m = tx.note?.match(/bsc_tx_(\w+)/); return m ? m[1] : null; })
        .filter(Boolean)
    );

    // Filtra: destino correcto + remetente == wallet do user + sem erro + não processada
    const eligible = txs.filter(tx =>
      tx.to?.toLowerCase()   === CONFIG.bscReceiverAddr.toLowerCase() &&
...
async function requestBscWithdrawal(userId, toAddress, amount) {
  // Sem chave privada configurada → fallback manual (admin processa na BSC)
  try {
...
async function processReferral(referrerTelegramId, newUserId, from) {
  ...
  const bonusBnb  = await usdToBnb(CONFIG.referralBonusUsd);
  const bnbPrice  = bnbPriceCached();
  ...
  const faltam = CONFIG.minRefsWithdraw - novoCount;
  await sendMessage(referrerTelegramId,
    `🎉 *Nova referência!*\n\n` +
    `@${from.username || from.first_name || 'utilizador'} entrou pelo teu link.\n` +
    `💎 *+${bonusBnb} BNB* (~$${CONFIG.referralBonusUsd}) creditado!\n` +
    `📊 *${novoCount}/${CONFIG.minRefsWithdraw}* referências\n\n` +
...
const KB = {
  ...
  taskList: (tasks, page, total, showPromo = false) => {
    ...
    if ((page + 1) * (CONFIG?.tasksPerPage || TASKS_PER_PAGE) < total) nav.push({ text: 'Próxima ▶️', callback_data: `tasks_page_${page + 1}` });
...
async function handleStart(msg) {
  ...
  try {
    ...
    if (isNew) {
      await sendMessage(chatId,
        t(locale0, 'account_created', { bonus: CONFIG.referralBonusUsd, minRefs: CONFIG.minRefsWithdraw }),
...
async function handleSaldo(msg) {
  ...
  const minWithdrawBnb = await usdToBnb(CONFIG.minWithdrawUsd);
  const prog     = Math.min(Math.round((refs / CONFIG.minRefsWithdraw) * 10), 10);
  const bar      = '█'.repeat(prog) + '░'.repeat(10 - prog);
  const statusLine = refs >= CONFIG.minRefsWithdraw
    ? t(locale, 'balance_withdraw_ready')
    : t(locale, 'balance_need_more', { faltam: CONFIG.minRefsWithdraw - refs });
  ...
  await sendMessage(msg.chat.id,
    ...
    `💱 Preço BNB: *$${bnbPrice}*\n\n` +
    `${walletLine}\n\n` +
    `👥 Referências: *${refs}/${CONFIG.minRefsWithdraw}*\n${bar}\n\n` +
    `Mínimo de saque: *${minWithdrawBnb} BNB* (~$${CONFIG.minWithdrawUsd})\n\n` +
    statusLine,
    {
      parse_mode: 'Markdown',
      reply_markup: {
...
async function handleSacar(msg) {
  ...
  const refs = user.referral_count || 0;
  if (refs < CONFIG.minRefsWithdraw) {
    return sendMessage(msg.chat.id,
      t(locale, 'withdraw_blocked', { minRefs: CONFIG.minRefsWithdraw, refs, faltam: CONFIG.minRefsWithdraw - refs }),
      { parse_mode: 'Markdown', reply_markup: KB.backToMenu(locale) }
    );
  }
...
async function handleTarefas(msg, page = 0) {
  const user      = await getOrCreateUser(msg.from);
  const perPage   = CONFIG.tasksPerPage || TASKS_PER_PAGE;
...
async function handleCriar(msg) {
  const user       = await getOrCreateUser(msg.from);
  const listingBnb = await usdToBnb(CONFIG.listingFeeUsd);
  const bnbPrice   = bnbPriceCached();
  if ((user.balance || 0) < listingBnb) {
    return sendMessage(msg.chat.id,
      `❌ *Saldo insuficiente*\n\n` +
      `Taxa de listagem: *${listingBnb} BNB* (~$${CONFIG.listingFeeUsd})\n` +
...
async function handleReferral(msg) {
  ...
  const earnedUsd = (refs * CONFIG.referralBonusUsd).toFixed(2);
  const prog    = Math.min(Math.round((refs / CONFIG.minRefsWithdraw) * 10), 10);
  const bar     = '█'.repeat(prog) + '░'.repeat(10 - prog);
  ...
  await sendMessage(msg.chat.id,
    ...
    `${bar} *${Math.round((refs / CONFIG.minRefsWithdraw) * 100)}%*\n` +
    `📊 *${refs}/${CONFIG.minRefsWithdraw}* referências\n` +
    `💎 *${earned} BNB* (~$${earnedUsd}) ganhos` +
    leaderboard +
    `\n\n_Cada referência = +${bonusBnb} BNB (~$${CONFIG.referralBonusUsd})_\n` +
    `_${CONFIG.minRefsWithdraw} refs para sacar mínimo $${CONFIG.minWithdrawUsd}_\n` +
...
async function handleMinhas(msg) {
  ...
  if (anunciante?.length)
    text += `*Como Anunciante:*\n` +
      anunciante.map(tk => `${statusEmoji(tk.status)} [#${tk.id}] ${tk.title} — ${tk.reward} BNB`).join('\n') + '\n\n';
  if (executor?.length)
    text += `*Como Executor:*\n` +
      executor.map(tk => `${statusEmoji(tk.status)} [#${tk.id}] ${tk.title} — ${tk.reward} BNB`).join('\n');
...
async function handleAjuda(chatId, from) {
  ...
  await sendMessage(chatId,
    t(locale, 'help_text', { minRefs: CONFIG.minRefsWithdraw, listingFee: `$${CONFIG.listingFeeUsd}` }),
    { parse_mode: 'Markdown', reply_markup: KB.backToMenu(locale) }
  );
}
...
async function handleWithdrawFSM(msg) {
  ...
  if (state.step === 'withdraw_amount') {
    ...
    const minWithdrawBnb = state.minWithdrawBnb || await usdToBnb(CONFIG.minWithdrawUsd);
    const bnbPrice     = bnbPriceCached();

    if (isNaN(amount) || amount < minWithdrawBnb) {
      await sendMessage(chatId,
        `❌ Valor inválido.\nMínimo: *${minWithdrawBnb} BNB* (~$${CONFIG.minWithdrawUsd} @ $${bnbPrice}/BNB)`,
        { parse_mode: 'Markdown', reply_markup: KB.withdrawCancel() });
      return true;
    }
    if (amount > saldo) {
...
async function handleCreateFSM(msg) {
  ...
  switch (state.step) {
    ...
    case 'create_slots': {
      ...
      const user       = await getUser(msg.from.id);
      const listingBnb = state.listingBnb || await usdToBnb(CONFIG.listingFeeUsd);
      const bnbPrice   = bnbPriceCached();
      ...
      await sendMessage(chatId,
        ...
        `🏷 Taxa: ${listingBnb} BNB (~$${CONFIG.listingFeeUsd})\n` +
        `━━━━━━━━━━━━━━━━\n` +
...
async function cancelTask(chatId, msgId, from, taskId) {
  ...
  await editMessage(chatId, msgId,
    t(lang(from), 'task_cancelled', { refund: refund.toFixed(6), fee: CONFIG.listingFeeUsd }),
    { parse_mode: 'Markdown', reply_markup: KB.backToMenu() });
}
...
async function handleCallback(cb) {
  ...
  // Responde sempre ao callback para evitar botões "a girar"
  if (!data.startsWith('dep_check|')) await answerCallback(cb.id).catch(() => {});
  ...
  // ── Admin ────────────────────────────────────────────────────────────
  // ── Painel Admin ──────────────────────────────────────────────────────
  if (data === 'adm_menu')          { if (!isAdmin(from.id)) return answerCallback(cb.id, '⛔ Sem permissão.', true); return handleAdmin({ chat: { id: chatId }, from }); }
  if (data === 'adm_relatorio')     { if (!isAdmin(from.id)) return answerCallback(cb.id, '⛔ Sem permissão.', true); return handleRelatorio({ chat: { id: chatId }, from }); }
  if (data === 'adm_users')         { if (!isAdmin(from.id)) return answerCallback(cb.id, '⛔ Sem permissão.', true); return handleAdminUsers(chatId); }
  if (data === 'adm_tasks')         { if (!isAdmin(from.id)) return answerCallback(cb.id, '⛔ Sem permissão.', true); return handleAdminTasks(chatId); }
  if (data === 'adm_config')        { if (!isAdmin(from.id)) return answerCallback(cb.id, '⛔ Sem permissão.', true); return handleAdminConfig(chatId); }
  if (data === 'adm_withdrawals')   { if (!isAdmin(from.id)) return answerCallback(cb.id, '⛔ Sem permissão.', true); return handleAdminWithdrawals(chatId); }
  if (data === 'adm_withdrawals_clear') {
    if (!isAdmin(from.id)) return answerCallback(cb.id, '⛔ Sem permissão.', true);
  ...
  if (data.startsWith('create_type_')) {
    ...
    const state = getState(from.id);
    if (!state || state.step !== 'create_type') {
      return answerCallback(cb.id, '⏱ Sessão expirada. Usa /criar novamente.', true);
    }
  ...
  if (data.startsWith('dispute_')) return handleDisputeCallback(cb, data);
    ...
    sendMessage(chatId, '❌ Erro interno. Tenta novamente.').catch(() => {});
  }
}
...
async function handleAdminTasks(chatId) {
  ...
  const lines = tasks.map(tk =>
    `#${tk.id} ${statusEmoji(tk.status)} ${taskTypeEmoji(tk.task_type)} *${tk.title}*\n` +
    `   💎 ${tk.reward} BNB · ${tk.slots_remaining}/${tk.total_slots} vagas`
  ).join('\n\n');
