...
async function creditDepositIdempotent(userId, amount, invoiceId) {
  ...
  await logTx(userId, 'deposit', amount, `Depósito BNB · ${invoiceId}`);
...
const KB = {
  ...
  depositBsc: (userId, memo) => ({
    inline_keyboard: [
      [{ text: '🔄 Verificar depósito', callback_data: `dep_check_bsc_${userId}_${memo}` }],
...
async function checkBscDeposit(userId, memo, chatId) {
  try {
    ...
    // Procura txs recentes (últimas 200 blocos ≈ ~10min) para este utilizador
    // O memo é incluído pelo utilizador no campo `input` da tx (hex) ou verificamos
    // todas as txs recentes para o endereço receptor e creditamos pelo timestamp
    const { data: pendingInv } = await supabase.from('deposit_invoices')
      .select('*').eq('invoice_id', memo).eq('status', 'pending').maybeSingle();
    if (!pendingInv) return false; // já foi pago ou não existe

    // Verifica se há txs não processadas para este endereço
    const { data: processedTxs } = await supabase.from('transactions')
      .select('note').eq('user_id', userId).ilike('note', '%bsc_tx_%');
    const processedHashes = new Set((processedTxs || []).map(t => {
      const m = t.note?.match(/bsc_tx_(\w+)/);
      return m ? m[1] : null;
    }).filter(Boolean));

    // Filtra txs recebidas não processadas
    ...
    // Credita a tx mais recente não processada a este utilizador
    // (em produção deves associar o memo à tx via input data)
    const tx = unprocessed[0];
    const amountBnb = parseFloat(tx.value) / 1e18;
    if (amountBnb <= 0) return false;
    ...
    console.error('[checkBscDeposit]', e.message);
    return false;
  }
}
