...
//   /inicio      — substituto robusto de /start (mostra menu sem falhar)
...
async function handleStart(msg) {
  ...
  try {
    ...
    console.error('[/start] ❌', err.message, err.code || '');
    // Tenta mostrar menu mesmo com erro de registo
    try {
      await sendMessage(chatId,
        `⚠️ Houve um problema ao carregar o teu perfil.\n\nTenta /inicio para aceder ao menu directamente.`,
...
//   3. Existente com ref     → ignora (anti-fraude)
async function handleInicio(msg) {
  const chatId = msg.chat.id;
  const from   = msg.from;

  // Aceita payload de /inicio r<id> e também deep links /start r<id>
  const rawPayload = (msg.text || '').trim().split(/\s+/).slice(1).join('');
  const referrerId = (rawPayload && /^r\d+$/.test(rawPayload))
    ? parseInt(rawPayload.slice(1), 10)
    : null;

  console.log(`[/inicio] id=${from.id} @${from.username || '—'} ref=${referrerId || '—'}`);

  clearState(from.id);
  const nome = from.first_name || 'utilizador';

  // Mostra menu imediatamente — não espera pela BD
  await sendMessage(chatId,
    `👋 Olá, *${nome}*!\n\nO que queres fazer?`,
    { parse_mode: 'Markdown', reply_markup: KB.mainMenu() }
  ).catch(() => {});

  // Registo / referral em background
  try {
    let user = await getUser(from.id);
    ...
    if (!user) {
      ...
      if (referrerId && referrerId !== from.id) {
        await processReferral(referrerId, user.id, from);
      }
      await sendMessage(chatId,
        `✅ *Conta criada!*\n\n` +
        `📋 Completa tarefas e ganha *TON*\n` +
      ...
      // ── 2 & 3. Utilizador existente ─────────────────────────────
      await updateUserProfile(from);

      if (referrerId && referrerId !== from.id && !user.referred_by) {
        // Verifica duplicado na tabela referrals (anti-fraude)
        const { data: dup } = await supabase.from('referrals').select('id')
          .eq('referrer_id', referrerId).eq('referred_id', from.id).maybeSingle();

        if (!dup) {
          await supabase.from('users').update({ referred_by: referrerId })
            .eq('telegram_id', from.id);
          await processReferral(referrerId, user.id, from);
          console.log(`[/inicio] referral retroactivo: ${from.id} -> referrer ${referrerId}`);
        }
      }
    }
  } catch (err) {
    console.error('[/inicio:bg] ❌', err.message, err.code || '');
...
async function processUpdate(update) {
  if (update.message) {
    ...
    if (cmd !== '/start' && cmd !== '/inicio' && cmd !== '/ajuda' && cmd !== '/help') {
