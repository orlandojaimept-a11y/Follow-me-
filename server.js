...
async function handleDepositar(msg) {
  ...
  await sendMessage(msg.chat.id,
    ...
    `📝 *Memo/Referência:* \`${memo}\`\n` +
    `_(Inclui este memo na nota da transacção ou envia mensagem ao admin após o depósito)_\n\n` +
    `⚠️ *Atenção:*\n• Usa apenas a rede *BSC (BEP-20)*\n• Outros tokens/redes serão perdidos\n• O saldo é creditado após confirmação do admin`,
    {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
          [{ text: '✅ Já enviei — notificar admin', callback_data: `dep_notify_${user.id}_${memo}` }],
...
async function handleCallback(cb) {
  ...
  // Disputas
  // Depósito BSC — notificação manual
...
async function handleWithdrawFSM(msg) {
  ...
  if (state.step === 'withdraw_address') {
    ...
    await sendMessage(chatId,
      `💸 *Confirma o saque*\n\nValor: *${state.amount} BNB*\nRede: *BSC (BEP-20)*\nPara: \`${text}\`\n\n⚠️ Irreversível. O admin processa em 24h.`,
      {
        parse_mode: 'Markdown',
        reply_markup: {
          inline_keyboard: [
