...
async function handleSaldo(msg) {
  ...
  await sendMessage(msg.chat.id,
    t(locale, 'balance_title', { saldo, refs, minRefs: MIN_REFS_WITHDRAW, bar, statusLine }),
    {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [
...
async function handleSacar(msg) {
  ...
  setState(msg.from.id, { step: 'withdraw_amount', locale: locale_sacar });
  await sendMessage(msg.chat.id,
    t(locale_sacar, 'withdraw_title', { saldo: (user.balance || 0).toFixed(4) }),
